      ...
      <Card>
        <CardContent className="p-4">
          <div className="flex items-center gap-3 mb-2">
            ...
            <span className="text-sm font-medium text-gray-700">Phase Distribution</span>
            <span className="text-xs text-gray-400 ml-auto">{roadmap.total_items} total items</span>
          </div>
          ...
          <div className="flex gap-4 mt-2">
            {roadmap.phases.map((phase, i) => (
              <div key={phase.phase} className="flex items-center gap-1.5 text-xs text-gray-500">
            ...
            <div key={phase.phase} className="flex flex-col">
              <div className={`rounded-t-xl ${cfg.headerBg} ${cfg.borderColor} border-2 border-b-0 p-4`}>
                <div className="flex items-center gap-2 mb-1">
                  ...
                  <h2 className={`font-bold ${cfg.color}`}>{phase.name}</h2>
                </div>
                <div className="flex items-center justify-between">
                  <span className="text-xs text-gray-500">Weeks {phase.weeks}</span>
                  <span className="text-xs font-medium text-gray-600">{phase.items.length} items</span>
                </div>
                <div className="flex items-center gap-1 mt-2">
                  ...
                  <span className="text-[10px] text-gray-400">projected ROI</span>
                </div>
              </div>
      ...
      {item.jim_principles.length > 0 && (
        <div className="flex flex-wrap gap-1">
          {item.jim_principles.map((p) => (
            <span
              key={p}
              className="inline-flex items-center gap-0.5 text-[9px] rounded px-1.5 py-0.5 bg-amber-50 text-amber-700"
            >
              <Sparkles size={8} />
              {p.replace(/_/g, " ")}
            </span>
